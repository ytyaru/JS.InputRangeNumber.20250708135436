class HTMLInputRangeNumberElement extends HTMLElement {
    ...
    set value(v) {
        ...
        if ('number'!==typeof v) {v=parseFloat(v)}
        if (v < parseFloat(this._.min) || parseFloat(this._.max) < v) {return}
        'number range'.split(' ').map(n=>this._.el[n].value=v)
    }
    //get min() {return this._.d.s.min}
    get min() {return this._.el.number.min}
    set min(v) {this.#set('min')}
    //get max() {return this._.d.s.max}
    get max() {return this._.el.number.max}
    set max(v) {this.#set('max')}
    //get step() {return this._.d.s.step}
    get step() {return this._.el.number.step}
    set step(v) {this.#set('step')}
    #set(t) {
        //if (!this.#setN(v) && !this.#setS(v)) {throw new TypeError('NumberかString型のみ有効です。')}
        if (this.#setN(v) || this.#setS(v)) {'number range'.split(' ').map(n=>this._.el[n].value=v); return true}
        else {return false}
        /*
        if ('number'===typeof v && !Number.isNaN(v) && v!==this._.d.n[t]) {
            this._.d.n[t] = v;
            this._.d.s[t] = v.toString();
        } else if ('string'===typeof v || v instanceof String && v!==this._.d.s[t]) {
            this._.d.s[t] = v;
            this._.d.n[t] = Number(v);
        }
        */
    }
    #setN() {
    ...
    #setS() {
        if ('string'===typeof v || v instanceof String) {// && v!==this._.d.s[t]
            this._.d.s[t] = v;
            this._.d.n[t] = Number(v);
            return true
        } else {return false}
...
//            if (this._.el.host===e.detail.sender){return}
    ...
    #resizeNumber() {
        ...
        this._.el.number.style.width = `${parseFloat(getComputedStyle(this._.el.number).fontSize) * this._.d.s.max.length}px`;
    }
}
