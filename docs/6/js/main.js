...
//        van.tags.p('Create a WebComponent that combines the range and number of input elements.'),
    ...
    a.t(()=>{
        ...
        return one!==two && [one, two].every(el=>el instanceof HTMLNumBarElement);
    });
    a.t(()=>{
        const one = document.querySelector(`#one`);
        const two = document.querySelector(`#two`);
    ...
    // build
    a.t(()=>{
        const el = document.createElement(`num-bar`);
        const [blur, focus] = ['n', 'nR']
        const build = `${blur} ${focus}`;
        el.build = build;
        console.log(el._.build.s)
        console.log(el._.build.blur.s)
        console.log(el._.build.focus.s)
        return build===el._.build.s && blur===el._.build.blur.s && focus===el._.build.focus.s;
    });

    a.t(()=>{// build 'x y' 全パターン 144 = 12*12
        const el = document.createElement(`num-bar`);
        const res = [];
        const blurs = 'n r N R nr Nr nR NR rn Rn rN RN'.split(' ');
        const focuses = [...blurs];
        for (let blur of blurs) {
            for (let focus of focuses) {
                const build = `${blur} ${focus}`;
                el.build = build;
                res.push(build===el._.build.s && blur===el._.build.blur.s && focus===el._.build.focus.s);
            }
        }
        return res.every(r=>r);
    });

    a.t(()=>{// build 'x' 全パターン 12
        const el = document.createElement(`num-bar`);
        const res = [];
        const builds = 'n r N R nr Nr nR NR rn Rn rN RN'.split(' ');
        for (let build of builds) {
            el.build = build;
            res.push(build===el._.build.s && build===el._.build.blur.s && build===el._.build.focus.s);
        }
        return res.every(r=>r);
    });
